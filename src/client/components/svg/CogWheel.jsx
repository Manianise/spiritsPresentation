...
/**
 * 
 * @param {int} size is estimated in px
 * @param {string} orientation can make the wheel spin. If left, rotates left, if right, rotates right
 * @returns 
 */
export default function CogWheel( {children, ...props}) {

    const {fillColor = '#151f2e8e' } = props
    const {strokeColor = '#b55d41' } = props
    const {strokeSize = 10 } = props
    const {orientation = 'left'} = props

    return <div className="wrapper" id={props.id}>
            <div className={`cogwheel spin-${orientation}`} >
                <svg
                    style = {{
                        fill : fillColor,
                        stroke : strokeColor,
